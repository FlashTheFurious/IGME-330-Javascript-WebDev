...
function draw(params = {}) {
  ...
  if (params.showNoise) {
    ...
    // imageData.data' is a Vint8clampedArray(1280000) typed array that has 1.28 million elements!
    ...
    for (let i = 0; i < length; i += 4) {
      // C) randomly change every 28th pixel to purple
  ...
  // Show CircleBars - Currently, this overpowers other effects.
  //  needto modify in future
  if (params.showCircleBar) {
    ...
    ctx.translate(canvasWidth / 2, middleY - 75);
    for (let b of audioData) {
      let percent = b / 255;
      if (percent < 0.2) {
        percent = 0.2;
